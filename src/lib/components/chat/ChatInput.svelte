<script lang="ts">
  ...
  try {
    ...
    console.error(error);
    alert('An error occurred while fetching token balance. Please try again.');
  }
}
  ...
  async function purchaseCredits() {
    try {
      ...
      if (formattedCredits < 1) {
        ...
        const contract = new ethers.Contract(contractAddress, contractAbi, provider);
        const balance = await contract.balanceOf(account);
        const amount = ethers.utils.parseUnits("1", 18); // 1 GPT tokens
        ...
        if (balance.gte(amount)) {
          ...
          alert('Insufficient balance. Please acquire at least 1 $GPT token enable prompt credits to start a conversation.');
        }
      } else {
